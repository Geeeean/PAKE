...
int run_key_exchange(const unsigned char *password,
                               const unsigned char *client_id,
                               const unsigned char *server_id,
                               unsigned char phi0_s[32],
                               unsigned char c[32],
                               unsigned char key_client[32], unsigned char key_server[32])
{
    /*** CLIENT PAKE ***/
    ...
    if (H_prime(phi0_s, crypto_core_ristretto255_SCALARBYTES, (const unsigned char *)client_id,
                strlen((char *)client_id), (const unsigned char *)server_id,
                strlen((char *)server_id), u, sizeof(u), v, sizeof(v), w, sizeof(w), d,
                sizeof(d), key_server) != 0) {
...
void protocol_doesnt_produce_same_keys_with_same_credentials(void)
{
